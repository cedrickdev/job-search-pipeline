---
name: agentic-apply
description: Claude personally drives Playwright to submit job applications (not the automated Python/VLM worker) and confirms each one via Gmail before marking it applied. Opt-in only - invoke when the user explicitly asks Claude to apply itself (e.g. "use the agentic apply skill", "apply yourself to these jobs"). Never runs automatically as part of a normal morning run.
---
