...
// Canonical shape — matches the dicts returned by server/followups.py
// (applied_no_reply / recruiter_no_outbound). Phase 9.4 reuses this exact
// interface; it MUST NOT re-declare it (duplicate identifier in the same file).
...
// draft. mandate_ok=false + flags means it did not clear the gate (§6.2).
...
export interface PrepData {
  ...
  // Present only on a fresh /prep/generate response (spec §6.2 fail-closed): a
  // draft that did not clear the mandate gate has mandate_ok=false + flags and
...
// drive drawer navigation (`onOpen(job_id)`, consistent with NeedsYouItem),
// not `application_id` (which the frontend never needs — interview PATCH is
// keyed by the interview `id`).
