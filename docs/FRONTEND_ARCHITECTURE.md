...
## Providers (added in Phase 11)

One guarded screen — `pages/providers.vue` at `/providers`, a **new top-level route** rather than a page nested under the V1 `/settings` (which stays a V1 screen on V1 composables) — over the LLM connections API. It is fed by `composables/useLlmConnections.ts` (the `llm:connections` query plus the create/update/remove/enable/default/probe mutations) and built on a reusable `components/LlmConnectionForm.vue`. The full statement is [LLM Connections](./LLM_CONNECTIONS.md); the contracts that are contracts rather than preferences:

- **The credential is write-only, and the form proves it before the request does.** A stored key is only ever "key set" on screen, never a value; an edit of a connection that stores one shows keep/replace/remove radios, and a create or a keyless connection shows a plain optional add-key box where an empty box means "no credential", not an empty string. There is no field that renders a key.
- **A CLI type omits the endpoint, key and header fields entirely.** Not disabled — absent, because Claude Code and Codex carry none of them and the platform never injects a key into a self-authenticating CLI (§1). An API type reveals all three and will not submit without a base URL. Provider type is fixed as a label on an edit, because it is identity.
- **Health is a probe rendered as state, never stored.** Testing a connection is a mutation that invalidates nothing; a misconfigured local endpoint comes back as a `MISCONFIGURED` status the row shows, not a thrown error. The seventeen new provider/connection error slugs each map to their own sentence in `utils/v2-errors.ts`, so a refusal is one line beside the form.
- **The screen is session-gated.** The `Providers` nav link appears only for a signed-in visitor, and `middleware: 'auth'` sends an anonymous click to `/login?redirect=/providers` — the connections are user-owned, unlike the unconditional V1 screens.

The suite grew again: **454** Vitest specs across 48 files in `frontend/tests/nuxt/` and **41** Playwright flows in `frontend/tests/e2e/`, still with every `/api/**` request stubbed in the browser and every test asserting that nothing went unstubbed. The three new browser flows exist for what only a browser can show: that the screen is reachable from the nav and shows "key set" for a stored credential, that a CLI create leaves with its `X-CSRF-Token` header and no key field, and that the guard sends an anonymous visitor to `/login`. No unit or browser test touches a live LLM or provider: a connection is a stubbed payload and a healthcheck a stubbed response (CLAUDE.md §Testing).
